...
- 是否有未读明文消息
- 是否有 E2EE 握手被自动处理
- 哪些对端存在尚未解密的加密消息（需要 `--process` 或 WebSocket 监听器）
- JWT 是否需要刷新
...
| `len(inbox.e2ee_encrypted_from) > 0` | 通知有待解密的加密消息；使用 `e2ee_messaging.py --process --peer <DID>` 或 WebSocket 监听器处理 |
...
| 方案 | 延迟 | E2EE | 配置 | 最佳场景 |
|------|------|------|------|----------|
| **心跳**（推荐） | 最长 15 分钟 | 自动处理 `e2ee_init` / `e2ee_rekey` / `e2ee_error`；`e2ee_msg` 仍需 `e2ee_messaging.py --process` | 无——已完成 | 通用，所有通道 |
...
| 方式 | 工作原理 | 推荐？ |
|------|----------|--------|
| **心跳 + CLI** | `check_status.py --auto-e2ee` 自动处理 `e2ee_init` / `e2ee_rekey` / `e2ee_error`；未读 `e2ee_msg` 仍需 `e2ee_messaging.py --process` | 默认——全平台适用 |
...
| 操作 | 描述 | 优先级 |
|------|------|--------|
| **查看仪表盘** | `check_status.py --auto-e2ee`——一览身份、收件箱、握手状态和待解密来源 | 🔴 首先执行 |
